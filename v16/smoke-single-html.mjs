...
if (!existsSync(htmlPath)) {
  ...
  try {
    ...
    check('single html replaces native confirm dialogs', !script.includes('window.confirm') && !script.includes('alert(') && script.includes('data-confirm-modal') && script.includes('openConfirmDialog') && script.includes('requestDeleteConfirm'), '');
    check('single html confirms destructive actions in app UI', script.includes('data-confirm-run') && script.includes('data-confirm-cancel') && script.includes('confirm-detail') && script.includes('confirmDialog?.action'), '');
    ...
    check('single html shows supabase db coverage summary', script.includes('data-v15-db-coverage') && script.includes('Supabase table coverage') && script.includes('V15_DB_TABLE_LABELS'), '');
    check('single html shows supabase db table errors', script.includes('data-v15-db-table-errors') && script.includes('Supabase table errors') && script.includes('data-diagnostics-v15-db-table-errors'), '');
    check('single html warns on partial supabase db coverage', script.includes('data-v15-db-readiness-warning') && script.includes('Live data partial') && script.includes('Supabase'), '');
    ...
    check('single html previews supabase audit rows', script.includes('data-v15-audit-preview') && script.includes('Supabase audit preview') && script.includes('No v15 audit rows loaded.'), '');
